...
const Slide = ({ current, view, slide, max }) => {
    ...
    return ( 
        <div className={`xs:w-full lg:w-128 slugish-transition centered-col absolute top-0 left-0 lg:left-[25%]  ${isVisible ? 'opacity-1 translate-x-0': `opacity-0 ${distance[status]}`}`}>
            <div className={`bg-img bg-cover w-20 h-20 translate-y-[50%] ${slideImg[slide]}`} />
            <article className="bg-lightgray w-full p-8 xs:text-center centered-col gap-y-4 xs:h-96 lg:h-72 lg:pt-20">
